...
class Doctor(UserMixin, db.Model):
    ...
    def create_user_directory(self):
        ...
        # Абсолютный путь относительно корня проекта, чтобы не зависеть от текущей рабочей директории
        project_root = Path(__file__).resolve().parents[3]
        user_dir = project_root / "data" / "studies" / self.username
        user_dir.mkdir(parents=True, exist_ok=True)

    def get_user_directory(self):
        """Получение пути к папке врача"""
        project_root = Path(__file__).resolve().parents[3]
        return project_root / "data" / "studies" / self.username
